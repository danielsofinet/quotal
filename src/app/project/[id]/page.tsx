...
export default async function ProjectDetailPage({
  ...
  // On-demand normalization: run once when >=2 DONE quotes have un-normalized items
  const doneQuotes = project.quotes.filter(
    (q) => q.processingStatus === "DONE" && q.lineItems.length > 0
  );
  const hasUnnormalized = doneQuotes.some((q) =>
    q.lineItems.some((li) => li.canonicalName === null)
  );

  if (hasUnnormalized && doneQuotes.length >= 2) {
    try {
      await normalizeProjectItems(id);
      // Refetch quotes with updated canonical names
  ...
  const pendingCount = project.quotes.filter(
    (q) => q.processingStatus === "PENDING" || q.processingStatus === "PROCESSING"
  ).length;
    ...
    >
      ...
      <div className="mb-6">
        ...
        <p className="text-text-muted text-sm mt-1">
          ...
          {project.quotes.length === 1 ? "quote" : "quotes"}
          {pendingCount > 0 && (
